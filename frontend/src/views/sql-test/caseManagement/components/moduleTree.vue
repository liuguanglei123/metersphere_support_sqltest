...
<!--    以前这里有if条件控制，强制修改为true-->
    <template v-if="true">
      ...
      >
        <template #expandRight>
          ...
          >
            <MsButton v-if="true" type="icon" class="!mr-0 p-[2px]">
              <MsIcon
    ...
    <a-spin class="w-full" :loading="loading">
      <MsTree
        ...
        :draggable="hasAnyPermission(['PROJECT_API_DEFINITION:READ+UPDATE'])"
...
<script setup lang="ts">
  ...
  const props = withDefaults(
    defineProps<{
      ...
      // TODO：先把弹窗和只读的两个参数以及相关逻辑删掉，对于sql测试来说暂时用不到，等到后面增加分享功能时再考虑加进来
      // TODO：同样删除掉的还有分享模式和回收站模式
  ...
  const isExpandSql = ref(getLocalStorage('isExpandSql') === 'true');
