<template>
  <a-form ref="createFormRef" :model="scenario" layout="vertical">
    ...
    <a-form-item :label="t('apiScenario.status')" class="mb-[16px]">
      ...
      >
        <template #label>
          <sqlStatus :status="scenario.status" />
        </template>
        <a-option v-for="item of Object.values(SqlScenarioStatus)" :key="item" :value="item">
          <sqlStatus :status="item" />
        </a-option>
      </a-select>
    </a-form-item>
    ...
    <template v-if="props.isEdit">
      <a-form-item field="createUserName" :label="t('apiScenario.table.columns.createUser')" class="mb-[16px]">
        <a-input :model-value="(scenario as SqlScenarioDetail).createUserName" disabled />
      </a-form-item>
      <a-form-item field="createTime" :label="t('apiScenario.table.columns.createTime')" class="mb-[16px]">
        <a-input :model-value="dayjs((scenario as SqlScenarioDetail).createTime).format('YYYY-MM-DD HH:mm:ss')" disabled />
      </a-form-item>
      <a-form-item field="updateTime" :label="t('apiScenario.table.columns.updateTime')" class="mb-[16px]">
        <a-input :model-value="dayjs((scenario as SqlScenarioDetail).updateTime).format('YYYY-MM-DD HH:mm:ss')" disabled />
      </a-form-item>
    </template>
  </a-form>
</template>
...
<script setup lang="ts">
  ...
  import { SqlScenario, SqlScenarioDetail } from '@/models/sqlTest/scenario';
  ...
  const scenario = defineModel<SqlScenarioDetail | SqlScenario>('scenario', {
    required: true,
  });
