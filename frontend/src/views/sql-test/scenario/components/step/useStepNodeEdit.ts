...
import { getScenarioDetail } from '@/api/modules/sql-test/scenario';
import { useI18n } from '@/hooks/useI18n';
import { findNodeByKey, getGenerateId, mapTree } from '@/utils';

import type { SqlScenario, SqlScenarioStepConfig, SqlScenarioStepDetail, SqlScenarioStepItem } from '@/models/sqlTest/scenario';
...
export default function useStepNodeEdit({
   ...
   */
  function setQuickInput(step: SqlScenarioStepItem, dataKey: keyof SqlScenarioStepDetail) {
    const realStep = findNodeByKey<SqlScenarioStepItem>(steps.value, step.uniqueId, 'uniqueId');
    if (realStep) {
      activeStep.value = realStep as SqlScenarioStepItem;
    }
   ...
   * 步骤描述编辑按钮点击
   */
  function handleStepDescClick(step: SqlScenarioStepItem) {
   ...
   * 应用步骤描述更改
   */
  function applyStepDescChange(step: SqlScenarioStepItem) {
    const realStep = findNodeByKey<SqlScenarioStepItem>(steps.value, step.uniqueId, 'uniqueId');
    if (realStep) {
   ...
   */
  function handleStepContentChange($event: Record<string, any>, step: SqlScenarioStepItem) {
    const realStep = findNodeByKey<SqlScenarioStepItem>(steps.value, step.uniqueId, 'uniqueId');
    if (realStep) {
   ...
   * 步骤启用禁用切换
   */
  function handleStepToggleEnable(data: SqlScenarioStepItem) {
    const realStep = findNodeByKey<SqlScenarioStepItem>(steps.value, data.uniqueId, 'uniqueId');
    if (realStep) {
   ...
   * 步骤名称编辑按钮点击事件
   */
  function handleStepNameClick(step: SqlScenarioStepItem) {
   ...
   * 应用步骤名称更改
   */
  function applyStepNameChange(step: SqlScenarioStepItem) {
    const realStep = findNodeByKey<SqlScenarioStepItem>(steps.value, step.uniqueId, 'uniqueId');
    if (realStep) {
   ...
   * 刷新引用场景的步骤数据
   */
  async function refreshScenarioStepInfo(step: SqlScenarioStepItem, id: string | number) {
    try {
      loading.value = true;
  ...
  async function saveScenarioConfig() {
    if (activeStep.value) {
      const realStep = findNodeByKey<SqlScenarioStepItem>(steps.value, activeStep.value.uniqueId, 'uniqueId');
      if (realStep) {
        ...
        if (scenarioConfigForm.value.refType === ScenarioStepRefType.REF) {
          // 更新子孙步骤完全引用
          await refreshScenarioStepInfo(realStep as SqlScenarioStepItem, realStep.resourceId);
        } else {
          realStep.children = mapTree<SqlScenarioStepItem>(realStep.children || [], (child) => {
            // 更新子孙步骤-步骤引用
