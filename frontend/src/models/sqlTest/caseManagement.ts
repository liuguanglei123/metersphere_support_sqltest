...
import { ResponseDefinition,SqlExecuteRequestParams } from './common';
...
// 定义-获取模块树参数
export interface SqlDefinitionGetModuleParams {
  keyword: string;
  searchMode?: 'AND' | 'OR';
...
// 创建定义参数
export interface SqlDefinitionCreateParams extends SqlExecuteRequestParams {
  tags: string[];
...
// 更新定义参数
export interface SqlDefinitionUpdateParams extends Partial<SqlDefinitionCreateParams> {
  id: string;
...
// 定义-自定义字段详情
// TODO：暂时留着这些字段，还不清楚作用，后续根据实际情况调整或删除
export interface SqlDefinitionCustomFieldDetail {
  id: string;
  name: string;
  scene: string;
  type: string;
...
// 定义详情
export interface SqlDefinitionDetail extends SqlDefinitionCreateParams {
  id: string;
  name: string;
  num: number;
  pos: number;
  ...
  caseStatus: string;
  follow: boolean;
  customFields: SqlDefinitionCustomFieldDetail[];
}
...
// 定义列表查询参数
export interface SqlDefinitionPageParams extends TableQueryParams {
  id: string;
  name: string;
  projectId: string;
  versionId: string;
