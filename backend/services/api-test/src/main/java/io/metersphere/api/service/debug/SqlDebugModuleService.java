...
public class SqlDebugModuleService extends ModuleTreeService {
    ...
    public List<BaseTreeNode> getTree(String userId) {
        List<BaseTreeNode> fileModuleList = extSqlDebugModuleMapper.selectBaseByProtocolAndUser(userId);
        List<BaseTreeNode> baseTreeNodes = super.buildTreeAndCountResource(fileModuleList, true, Translator.get(UNPLANNED));
        List<SqlTreeNode> sqlTreeNodeList = extSqlDebugModuleMapper.selectSqlDebugByProtocolAndUser(userId);
    ...
    public List<BaseTreeNode> getBaseTreeNodes(List<SqlTreeNode> sqlTreeNodeList, List<BaseTreeNode> baseTreeNodes) {
        ...
        List<BaseTreeNode> nodeList = sqlTreeNodeList.stream().map(sqlTreeNode -> {
            ...
            if (StringUtils.equals(sqlTreeNode.getProtocol(), ModuleConstants.NODE_PROTOCOL_HTTP)) {
                baseTreeNode.putAttachInfo(METHOD, sqlTreeNode.getMethod());
                baseTreeNode.putAttachInfo(PROTOCOL, sqlTreeNode.getProtocol());
            } else {
                baseTreeNode.putAttachInfo(PROTOCOL, sqlTreeNode.getProtocol());
            }
            return baseTreeNode;
