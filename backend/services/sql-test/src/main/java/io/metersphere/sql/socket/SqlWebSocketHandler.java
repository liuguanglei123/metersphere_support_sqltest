...
@Component
@ServerEndpoint("/ws/sql/{reportId}")
public class SqlWebSocketHandler {
    /**
     * 连接成功响应
     */
    @OnOpen
    public void openSession(@PathParam("reportId") String reportId, Session session) {
        ...
        if (async != null) {
            async.sendText(JSON.toJSONString(new SocketMsgDTO(reportId, "", MsgType.SQL_CONNECT.name(), MsgType.SQL_CONNECT.name())));
