...
public class SqlUtils {
    ...
    public static List<String> parse(String sql, DbType dbType, boolean removeComment) {
        ...
        try {
            ...
            if (removeComment) {
                // TODO：这里面使用druid的Lexer做了一些处理，不确定对sql的执行有什么影响，待确认
...
//                    sql = updateNow(sql, dbType); 更新sql语句中的now()为CURRENT_TIMESTAMP，对dingodb来说不需要
...
//            sql = removeDelimiter(sql);
     ...
     * 使用提供的处理器拆分 SQL 语句。
     *
     * 该方法使用给定的 {@link SqlSplitProcessor} 将包含多个 SQL 语句的字符串拆分为单个语句。
     * 它还处理拆分结果中可能包含的残留内容，并将其作为整体添加到结果列表中。
     * 此外，它提供了一个选项来移除 SQL 语句中的注释。
     *
     * @param processor 用于拆分 SQL 语句的 {@link SqlSplitProcessor}。
     * @param sql 包含要拆分的 SQL 语句的字符串。
     * @param dbType 数据库类型，用于移除注释。
     * @param removeComment 是否移除 SQL 语句中的注释。
     * @return 返回拆分后的 SQL 语句列表，如果指定了移除注释，则返回没有注释的 SQL 语句。
     */
    ...
    // TODO:这里因为 jsqlparser 版本的问题，有些不兼容，jsqlparser必须和pagehelper版本完全一致才能运行，chat2db和metersphere使用的版本有差异
