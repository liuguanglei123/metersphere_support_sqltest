...
public class SqlSplitProcessor {
    ...
    /**
     * 将给定的SQL脚本分割成多个部分，每个部分都是一个SplitSqlString对象
     * 此方法主要用于处理和解析不同数据库类型的SQL脚本
     *
     * @param buffer StringBuffer对象，用于存储SQL脚本的修改版本
     * @param sqlScript 输入的SQL脚本字符串
     * @return 返回一个List，包含分割后的SQL脚本部分
     */
    public synchronized List<SplitSqlString> split(StringBuffer buffer, String sqlScript) {
        // 检查输入的SQL脚本是否为空，如果为空则返回一个空的ArrayList
        if (StringUtils.isBlank(sqlScript)) {
            return new ArrayList<>();
        }
        try {
            // 初始化一个列表，用于存储分割后的SQL脚本部分
            List<SplitSqlString> offsetStrings = new ArrayList<>();

            // 将SQL脚本按行分割，返回一个列表，每行都是一个OrderChar列表
            List<List<OrderChar>> lines = splitLine(sqlScript);
            // 初始化一个Holder对象，用于跟踪当前处理的字符顺序
...
//                    addLineMysql(offsetStrings, buffer, bufferOrder, item);
    ...
    /**
     * 将SQL脚本拆分成多行表示，每行包含一系列OrderChar对象
     * 此方法的目的是为了解析SQL语句，将它们按行分割成字符集合，便于后续处理
     *
     * @param sqlScript 待分割的SQL脚本字符串
     * @return 返回一个List，其中每个元素都是代表一行的OrderChar对象列表
     */
    ...
    /**
     * 将解析后的SQL行添加到MySQL格式的SQL列表中
     * 此方法主要负责处理和格式化SQL语句，包括处理分隔符、注释、字符串等，并将格式化后的SQL语句添加到列表中
     *
     * @param sqls 存储格式化后SQL语句的列表
     * @param buffer 用于临时存储SQL语句的字符串缓冲区
     * @param bufferOrder 用于记录当前SQL语句在原始文本中的位置
     * @param line 当前要处理的SQL行
     */
